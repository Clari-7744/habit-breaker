...
class SetupCog(commands.Cog):
    ...
    async def add(
        ...
        *,
        ...
        await cur.execute("SELECT * FROM config WHERE user_id=? AND habit=?", (ctx.author.id, fmt_habit(habit)))
        if await cur.fetchone():
            return await ctx.send(f"You already have a habit configured with the name `{habit}`")
        await cur.execute(
    ...
    async def log(
        self,
        ctx: DPyUtils.Context,
        override_time: Optional[int] = commands.Option(default=None, description="Override time that you'd like to log (UTC timestamp)"),
        *,
    ...
    @dm_or_slash()
    async def check(self, ctx: DPyUtils.Context, *, habit: str = commands.Option(description="The habit you'd like to check")):
        """
    ...
    async def hlist(self, ctx: DPyUtils.Context):
        ...
        """
        cur = await self.bot.db.cursor()
        await cur.execute("SELECT habit, pretty_name FROM config WHERE user_id=?", (ctx.author.id,))
        habits = await cur.fetchall()
        if not habits:
            return await ctx.send(f"You have no habits configured. Type `{ctx.prefix}add HABIT` to add one.", ephemeral=True)
        await ctx.send(embed=discord.Embed(title="Configured Habits", description=", ".join(f"`{p}`" for _,p in habits)), ephemeral=True)

    @commands.command(name='remove')
    @dm_or_slash()
    async def remove(self, ctx: DPyUtils.Context, *, habit: str = commands.Option(description="The habit you'd like to remove")):
        """
        Removes a configured habit.
        """
        cur = await self.bot.db.cursor()
        await cur.execute("DELETE FROM config WHERE user_id=? AND habit=? RETURNING pretty_name", (ctx.author.id, fmt_habit(habit)))
        if not await cur.fetchone():
            return await ctx.send(f"You have no habit configured with the name `{habit}`", ephemeral=True)
        await self.bot.db.commit()
